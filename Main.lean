...
def applets : List Applet :=
  [
    ...
    { name := "chmod",    run := Lentils.Chmod.run,    descr := "change file mode bits" },
    { name := "comm",     run := Lentils.Comm.run,     descr := "compare two sorted files" },
    { name := "cp",       run := Lentils.Cp.run,       descr := "copy files and directories" },
    ...
    { name := "ln",       run := Lentils.Ln.run,       descr := "make links between files" },
    { name := "logname",  run := Lentils.Logname.run, descr := "print user's login name" },
    { name := "ls",      run := Lentils.Ls.run,      descr := "list directory contents" },
    { name := "mkdir",    run := Lentils.Mkdir.run,    descr := "make directories" },
    { name := "mv",       run := Lentils.Mv.run,       descr := "move (rename) files" },
    { name := "nproc",    run := Lentils.Nproc.run,    descr := "print number of processing units" },
    { name := "nice",     run := Lentils.Nice.run,     descr := "run a program with modified scheduling priority" },
    { name := "nohup",    run := Lentils.Nohup.run,    descr := "run a command immune to hangups" },
    ...
    { name := "rm",       run := Lentils.Rm.run,       descr := "remove files or directories" },
    { name := "rmdir",    run := Lentils.Rmdir.run,    descr := "remove empty directories" },
    ...
    { name := "touch",    run := Lentils.Touch.run,    descr := "change file timestamps" },
    { name := "timeout",  run := Lentils.Timeout.run,  descr := "run a command with a time limit" },
    ...
    { name := "fmt",     run := Lentils.Fmt.run,     descr := "reformat paragraph text" },
    { name := "sum",     run := Lentils.Sum.run,     descr := "BSD checksum and block counts" },
    { name := "cksum",   run := Lentils.Cksum.run,   descr := "POSIX CRC checksum" },
