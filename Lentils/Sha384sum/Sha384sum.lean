/-
Sha384sum - IO wrapper for the sha384sum utility. 0BSD
-/

import Lentils.Common.IO.Native
import Lentils.Sha384sum.Logic

namespace Lentils.Sha384sum
  ...
  for line in lines do
    ...
    | some (expected, file) =>
      try
        ...
        let actual := formatHex (sha384 fileContent)
  ...
  for line in lines do
    ...
    | some (expected, file) =>
      try
        ...
        let actual := formatHex (sha384 content)
        if actual != expected then
          IO.print s!"{file}: FAILED\n"
          anyFailed := true
        else
  ...
  if hasCheck then
    ...
    if args.isEmpty then
      ...
      for file in args do
        try
          let content ← readAll (← openFileRead file)
          IO.print (formatHex (sha384 content) ++ "  " ++ file ++ "\n")
        catch _ =>
          failed := true
      if failed then return 1 else return 0

end Lentils.Sha384sum
